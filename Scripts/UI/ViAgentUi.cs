...
namespace ViAgents.Unity
{
    ...
    public class ViAgentUi : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            if (!showUI)
            {
                viAgent.keepLog = false;
                return;
            }
            viAgent.keepLog = true;
            ...
            if (GUI.Button(new Rect(95, 25, 95, 20), "Set"))
            {
                ...
                {
                    if (viAgent != null && viAgent.IsSleeping)
                    {
                        viAgent.Sense(new SensorData(Sensor.Schedule, "wakeUp", 10));
                    }

                    foreach (var member in GameObject.FindGameObjectsWithTag("Crowd"))
                    {
                        var vi = member.GetComponent<ViAgent>();
                        if (vi != null && vi.IsSleeping)
                        {
                            vi.Sense(new SensorData(Sensor.Schedule, "wakeUp", 10));
                        }
                    }
                }
            }
            ...
            if (viAgent.CurrentAction != null && viAgent.CurrentAction.BT != null)
            {
                GUI.Label(new Rect(10, 100, 300, 20),
                    string.Format("Action: {0} with {1}", viAgent.CurrentAction.sensorRequest,
                        viAgent.CurrentAction.BT.name));
            }

            GUI.Box(new Rect(5, 125, 190, 120), "Queue");

            var str = "";
            foreach (var d in viAgent.WorkQueue)
            {
            ...
            if (showLog)
            {
                ...
                showQueue = GUI.Toggle(new Rect(200, 5, 80, 25), showQueue, "Queue");
                showSensor = GUI.Toggle(new Rect(280, 5, 80, 25), showSensor, "Sensor");

                var filtered = viAgent.log.Where((l) =>
                    showActions && l.Source == LogSource.Action ||
                    showQueue && l.Source == LogSource.Queue ||
                    showSensor && l.Source == LogSource.Sensor).Select(l => l.Message).ToArray();
